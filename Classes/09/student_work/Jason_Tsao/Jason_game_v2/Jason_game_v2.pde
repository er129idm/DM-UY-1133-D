...
void setup() {
  ...
  size(1080, 1080);
  //player positions
  player1 = new Players(30, 30, true);
  player2 = new Players(30, 970, false);
  //creating new traps 
  for (int i = 0; i < totalTraps; i++) {
    bombs.add(new Traps());
  }
  // creating new treasure 
...
void draw() {
  ...
  for (int i = 0; i < bombs.size(); i++) {
    ...
    for (int f = 0; f < goal.size(); f++) {
      goal.get(f).checkOverlap(player1.playerXPosition, player1.playerYPosition, 1);
      goal.get(f).checkOverlap(player2.playerXPosition, player2.playerYPosition, 2);
      goal.get(f).display();
    }
  }
